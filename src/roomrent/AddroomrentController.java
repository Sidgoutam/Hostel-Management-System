 ...
 * @author govind
 */
public class AddroomrentController implements Initializable {
    ...
    @FXML
    private void confirmrent(ActionEvent event) throws ClassNotFoundException, SQLException, IOException {
        try
        {
        ...
        String chk = "SELECT * FROM `pendrent` WHERE reg='"+rg+"' AND month = '"+mt+"' AND year ='"+yr+"'";
        ...
        else
        {
            ...
            System.out.print(ld);
        String query1 = "INSERT INTO `pendrent`(`reg`, `month`, `year`, `room rent`, `fine`, `total rent`,`last date`) VALUES (?,?,?,?,?,?,?)";
         ...
         alert1.setContentText("reg no "+reg.getText()+" room rent "+total+"ADDED");
         alert1.showAndWait();
             Parent s1 = FXMLLoader.load(getClass().getResource("/basic/addrent.fxml"));
    ...
    @FXML
    private void cancelrent(ActionEvent event) {
        add.setVisible(true);
        confirm.setVisible(false);
        cancel.setVisible(false);
