 ...
 * @author govind
 */
public class PendingroomrentController implements Initializable {
...
else
{
  ...
  doc.add(para2);
  Paragraph para3 = new Paragraph("ROOM RENT Pending \n\n",f3);
...
//To change body of generated methods, choose Tools | Templates.
  ...
  data.clear();
  SearchField.clear();
  ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `pendrent`");
  int start =1;
  while (rs.next())
  {
  ...
  data.add(new rent(start,Integer.parseInt(rs.getString(2)),name,hostno,roomno,rs.getString(3),Integer.parseInt(rs.getString(4)),rs.getInt(6),rs.getInt(7),rs.getString(8)));
  start+=1;
  }
    }
